  ...
  return (
    <div className="randomizer-container">
          ...
          {names.map((name, idx) => {
            return (
              <div className="randomizer-names-wrapper" key={idx}>
                <div className="randomizer-names-name">{name}</div>
              </div>
            );
          })}
        </div>
      ...
      <div className="randomizer-results-container">
        <div>
          <h1 className="randomizer-results-heading">Results</h1>

          <div className="randomizer-results-wrapper">
            {results.map((pair, idx) => {
              return (
                <div
                  className="randomizer-results-pair-wrapper"
                  key={`pair-group-${idx}`}
                >
                  ...
                  <div>
                    {pair.map((name, idx) => {
                      return (
                        <div
                          className="randomizer-results-pair-text"
                          key={`pair-group-name-${idx}`}
                        >
                          {name}
                        </div>
                      );
